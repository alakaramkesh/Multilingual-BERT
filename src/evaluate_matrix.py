...
def main():
    ...
    # Use the same tokenizer and dynamic padding setup as training.
    ...
    for train_lang in tqdm(languages, desc="Evaluating trained models"):
        ...
        # Load the model trained on one source language.
        model = AutoModelForTokenClassification.from_pretrained(str(model_dir))
        args = make_eval_args(
            params,
            str(models_dir / train_lang / "eval_tmp"),
        )
        trainer = Trainer(
            model=model,
            args=args,
        ...
        # Test the current model on all target languages.
        for test_lang in languages:
            ...
            rows.append(row)
            print(row)
            # Clear unused CUDA memory between evaluations.
    ...
    # Convert long-format results into a 5 x 5 transfer matrix.
    matrix = df.pivot(
        index="train_lang",
        columns="test_lang",
        values="accuracy",
    ...
    # Save a markdown version for the report.
    md = "# Transfer matrix\n\n"
...
def evaluate_dataset(model, dataset, data_collator, batch_size, device):
    # Evaluate accuracy without computing loss.
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Keep real labels only, and skip labels outside the model head.
            mask = (labels != -100) & (labels >= 0) & (labels < num_labels)
...
def main():
    ...
    for train_lang in tqdm(languages, desc="Evaluating trained models"):
        model_dir = models_dir / train_lang / "best"
        print(f"\nLoading model trained on {train_lang}: {model_dir}")

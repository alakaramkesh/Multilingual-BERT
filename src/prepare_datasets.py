...
def collect_all_labels(params):
    # Collect labels from all languages and splits, so no label is missing later.
    labels = {"<pad>"}
...
def save_label_map(labels, output_file):
    # Use contiguous ids for real labels and reserve -100 for <pad>.
    real_labels = sorted(label for label in labels if label != "<pad>")
    label_to_id = {label: idx for idx, label in enumerate(real_labels)}
    label_to_id["<pad>"] = -100
    id_to_label = {str(idx): label for label, idx in label_to_id.items() if idx != -100}
...
def main():
    ...
    print(
        q18_table[["language", "train_sentences", "train_tokens"]]
        .sort_values("train_sentences")
